...
type TransferQuery struct {
	...
	withFromAccount *UserAccountQuery
	withToAccount   *UserAccountQuery
	withFKs         bool
...
func (tq *TransferQuery) QueryFromAccount() *UserAccountQuery {
...
func (tq *TransferQuery) QueryToAccount() *UserAccountQuery {
	query := (&UserAccountClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (tq *TransferQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (tq *TransferQuery) WithFromAccount(opts ...func(*UserAccountQuery)) *TransferQuery {
	query := (&UserAccountClient{config: tq.config}).Query()
...
func (tq *TransferQuery) WithToAccount(opts ...func(*UserAccountQuery)) *TransferQuery {
	query := (&UserAccountClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TransferQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transfer, error) {
	...
	if query := tq.withFromAccount; query != nil {
		if err := tq.loadFromAccount(ctx, query, nodes, nil,
			func(n *Transfer, e *UserAccount) { n.Edges.FromAccount = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withToAccount; query != nil {
		if err := tq.loadToAccount(ctx, query, nodes, nil,
			func(n *Transfer, e *UserAccount) { n.Edges.ToAccount = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TransferQuery) loadFromAccount(ctx context.Context, query *UserAccountQuery, nodes []*Transfer, init func(*Transfer), assign func(*Transfer, *UserAccount)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transfer)
	for i := range nodes {
		if nodes[i].user_account_outgoing_transfers == nil {
			continue
		}
		fk := *nodes[i].user_account_outgoing_transfers
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_account_outgoing_transfers" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TransferQuery) loadToAccount(ctx context.Context, query *UserAccountQuery, nodes []*Transfer, init func(*Transfer), assign func(*Transfer, *UserAccount)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transfer)
	for i := range nodes {
		if nodes[i].user_account_incoming_transfers == nil {
			continue
		}
		fk := *nodes[i].user_account_incoming_transfers
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_account_incoming_transfers" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
